...
- [ ] Open the dmg, drag Messenger.app to /Applications, launch via **right-click → Open**
      (ad-hoc signed; Gatekeeper blocks plain double-click on first run).
- [ ] App opens and shows the messenger.com login page.
...
- [ ] Log in fully, completing **2FA**. The 2FA/checkpoint pages render *inside* the app —
      never kicked to the browser.
- [ ] Quit fully (Cmd+Q), relaunch → **still logged in**.
...
- [ ] Download an image/file attachment → lands in `~/Downloads`, no save dialog.

## 4. Dock badge (plan Task 5)

- [ ] With unread conversations, the dock icon shows the unread count; reading clears it.
...
- [ ] Enable notifications in messenger.com settings (accept the in-page prompt).
- [ ] With the app unfocused, send yourself a message from another device → a **native macOS
...
- [ ] 1:1 **voice call**: mic permission prompt → allow → two-way audio.
...
- [ ] `git push -u origin feat/rebuild-v1` → confirm the GitHub Actions CI run goes green
